...
public class StudentService {
    ...
    public void createStudent(final Student student) {
        ...
        if (studentRepository.getStudentByPesel(student.getPesel()) != null) {
            LOGGER.warn("createStudent() Student with a pesel {} found, throwing StudentAlreadyExistsException",
                    student.getPesel());

            throw new StudentAlreadyExistsException(
                    String.format("Student with pesel %s already exists", student.getPesel()));
        }
    ...
    public String deleteStudent(final String pesel) {
        ...
        return String.format("Student with pesel %s deleted successfully", pesel);
    }
    ...
    public String assignStudentToClass(final String class_id, final String pesel) {
        ...
        return String.format("Student with pesel %s assigned to class %s successfully", pesel, class_id);
    }

    private boolean checkIfStudentExists(final String pesel) {
        studentRepository.findById(pesel)
                .orElseThrow(() -> new StudentNotFoundException(
                        String.format("Student with pesel %s doesn't exist", pesel)));
        return true;
    }
    ...
    private boolean checkIfClassExists(String class_id) {
        Optional.ofNullable(classRepository.getClassByClassId(class_id))
                .orElseThrow(() -> new ClassNotFoundException(
                        String.format("Class with id %s doesn't exist", class_id)));
        return true;
    }
}
